...
@testset "AtomGraph" begin
    ...
    path_mp195 = abspath(@__DIR__, "strucs", "mp-195.cif")
    ...
    @testset "construct object" begin
        ...
        @test_throws ArgumentError AtomGraph(
            build_graph(abspath(@__DIR__, "strucs", "nanlaplstruc.cif"))...,
        )
    end
    ...
    @testset "save/load" begin
        ...
        ag = AtomGraph(g, ["C", "C", "C"])
        serialize(abspath(@__DIR__, "testgraph.jls"), ag)
        ag2 = deserialize(abspath(@__DIR__, "testgraph.jls"))
        @test adjacency_matrix(ag.graph) == adjacency_matrix(ag2.graph)
        @test elements(ag) == ag2.elements
        @test AtomGraphs.normalized_laplacian(ag) == ag2.laplacian
    end

    @testset "batch processing" begin
        file_list = readdir(abspath(@__DIR__, "strucs"), join = true)
    ...
    @testset "visualize" begin
        ...
        @test AtomGraphs.lt_edge(e_adj[2], e_adj[3]) == true # e1.src < e2.src
        @test AtomGraphs.lt_edge(e_adj[1], e_adj[2]) == true # e1.dest < e2.dest
        @test AtomGraphs.lt_edge(e_adj[3], e_adj[2]) == false
        adj = Float32.([0 1 1; 1 0 1; 1 1 0])
        g = SimpleWeightedGraph{Int32}(adj)
        ag = AtomGraph(g, ["C", "C", "C"])
        @test AtomGraphs.graph_edgewidths(ag) == [1.0, 1.0, 1.0]

        # test graph_colors
        ag = AtomGraph(g, ["O", "C", "O"])
        c1, c2, c3 = AtomGraphs.graph_colors(elements(ag))
        @test c1 == c3 != c2
